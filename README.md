...
持仓行上有一列闹钟图标（已经挂着条件单时带角标显示数量），点开就是条件单弹窗。弹窗按**你想干什么**分四页，而不是让人先从六个类型名里挑一个：

| 页 | 覆盖的类型 | 要填什么 |
|---|---|---|
| 保护持仓 | `stop_loss` + `take_profit` | 止损、止盈各一个开关，可以只开一个，也可以两个一起开 |
| 涨停破板卖 | `limit_up_break` | 只填卖出比例，触发价是当天动态算的涨停价 |
| 条件买入 | `buy_dip` / `buy_breakout` | 下探还是突破，二选一 |
| 涨停买入 | `limit_up_buy` | 只填买入数量 |

触发价有两种填法，随时切换：**绝对价格**，或**涨跌幅百分比**（按创建那一刻的昨收换算成绝对价存下来）。止损那行还有个「保本」按钮，一键填成本价 —— 跌破成本就走。汇总视图下建不了条件单（它是挂在某个账号的持仓上的保护，「一次下发到所有账号」在这里说不通），会提示先选账号。

止损和止盈一起设，出来的是**两张各自独立的条件单**：分别触发、分别撤销，触发一张不会撤掉另一张。所以提交前弹窗会把「将创建几张、每张什么条件」摊开列出来 —— 设一次出两张单，不该是提交完才发现的事。

条件单列表在「买卖流水」下面，和「未成交委托」并排，每行一个撤销按钮。
