...
| 模块 | 能力 |
|---|---|
| **持仓与资产** | 实时持仓、资金曲线、累计/当日盈亏、交易日历、[出入金与盈亏矫正](#出入金与盈亏矫正) |
...
| **条件单** | 止损/止盈/条件买入/涨停破板卖出/涨停买入，6 种触发类型；持仓行点图标即可设置，服务端每 5 秒查一次价，越过触发线就走正常下单链路真的报单，见[条件单](#条件单) |
...
持仓行上点闹钟图标可以给这只票挂条件单——止损、止盈、涨停破板卖、条件买入、涨停买入。
止损和止盈可以一起设（出来是两张各自独立的单），触发价能填绝对价格也能填涨跌幅，
「保本」一键填成本价。提交前会把要建几张、每张什么条件摊开写清楚：

![条件单弹窗](docs/screenshots/08-conditional-dialog.png)

面板关掉也照常工作——盯价和报单都在服务端。详见[条件单](#条件单)。
...
- 382 个测试，不需要 QMT 也不需要任何密钥就能跑；下单链路用测试替身，风控与数量规整跑的是真实代码。
- 品种规则（最小申报量、步进、报价精度）与大QMT 自己的合约数据交叉验证过。
- **通知渠道配置页和国债逆回购开关还没做界面**，这两个目前只能改配置文件或调接口；**做T执行引擎尚未包含**。详见[尚未包含](#尚未包含)。
...
在持仓行上点一下就能给这只票挂止损/止盈，不用记接口。

### 从哪进去

持仓表的操作列里多了一个闹钟图标。**这只票已经挂着条件单时图标变橙并跟一个数字**——不用点开也知道有没有设防，设了几张：

![持仓行的条件单入口](docs/screenshots/07-conditional-column.png)

上图里湖南白银挂了 2 张（止损 + 止盈），浦发银行和中芯国际各 1 张，海南橡胶和沪深300ETF 没设。

条件单必须挂在**具体账号**上，所以汇总视图（「所有账号」）下建不了——它是给某个账号的持仓做的保护，「一次下发到所有账号」在这里说不通。这种情况弹窗会直接提示先选账号，而不是让你填完一堆再报错。

### 弹窗

![条件单弹窗](docs/screenshots/08-conditional-dialog.png)

顶上四个页签按**你想干什么**分，而不是让人先从六个类型名里挑一个：

| 页签 | 覆盖的类型 | 要填什么 |
|---|---|---|
| 保护持仓 | `stop_loss` + `take_profit` | 止损、止盈各一个开关，可以只开一个，也可以两个一起开 |
| 涨停破板卖 | `limit_up_break` | 只填卖出比例 |
| 条件买入 | `buy_dip` / `buy_breakout` | 下探还是突破，二选一 |
| 涨停买入 | `limit_up_buy` | 只填买入数量 |

图里的例子是同时开了止损和止盈：

- **止损**用「价格」模式填了 10.80。旁边的**「保本」按钮**一键把成本价填进来——跌破成本就走，不用自己去持仓表里抄那个数。汇总视图下没有这个按钮（那里的成本是几个账号合出来的加权值，当保本价没意义）。
- **止盈**用「涨跌幅」模式填了 15%。方向不用给负号，由类型本身决定（止损/下探是跌，止盈/突破是涨）。百分比在**创建那一刻**按当时的昨收价换算成绝对价存下来，之后不会跟着每天的昨收重新算。
- **触发时卖出 100%**——这个比例按**触发那一刻**的可用持仓现算，不是现在这个数。中间加过仓减过仓都算数。
- **触发后报价**只列市价类。这一项会被记住，下次开弹窗直接预选，而且和手动下单的记忆是分开的两套（下面有详细说明）。

最下面那块蓝色的是**提交前的方案预览**：会建出几张单、每张什么条件，全部摊开写清楚。止损和止盈一起设，出来的是**两张各自独立的条件单**——分别触发、分别撤销，触发一张不会撤掉另一张。这件事不该等到提交完才发现，所以放在按钮上面。

### 列表与撤销

条件单列表在「买卖流水」下面，和「未成交委托」并排：

![条件单列表](docs/screenshots/09-conditional-list.png)

一行一条，卖出方向绿、买入方向红（和面板其它地方一致）。「触发后卖/买」这一列会区分是按比例还是按固定数量，转债自动按「张」显示。每行一个撤销按钮，点了会二次确认。

触发之后这条就从列表里消失了，变成上面「未成交委托」里的一条真实委托——后续成没成交，看正常的委托和成交记录。
...
> **入口藏得比较深：点「累计盈亏」或「累计收益率」这两张卡片**，就会打开「累计盈亏矫正 (资金增减)」弹窗。卡片上没有按钮也没有图标，只有鼠标指针会变成手型（[#2](https://github.com/litaolemo/bigqmt_dashboard/issues/2)）。

面板拿不到真正的转账流水——大QMT 的接口只给资产快照，不给出入金事件。所以**一笔银证转账和一笔盈亏，在数据上长得一模一样**：总资产少了 2 万，面板没法自己判断是亏了 2 万还是转出了 2 万。

盈亏公式两边都会扣掉资金调整：

```
当日盈亏 = 当前总资产 - 昨末总资产 - 今日资金调整
累计盈亏 = 当前总资产 - 初始资金 - 累计资金调整(净流入)
```

所以**只要把出入金记进去，盈亏就是对的**；不记，转出的钱就会被算成亏损。正数表示转入/充值，负数表示转出/提现。

### 自动记录只覆盖一部分情况

`save_asset()` 会尝试自动补记，但门槛卡得很死，故意的：

| 条件 | 为什么 |
|---|---|
| 只在 **09:10-09:15** 和 **15:00-15:05** 取样 | 只有这两段总资产是静的，前后两次快照的差额除了出入金没有别的来源。09:15 那一分钟排除在外——集合竞价已经开始，价格在形成了 |
| 变动金额 > 1 万元 | 小额波动不值得猜 |
| 每个账号每天最多自动记一条 | 防止断线重连的坏数据反复插入巨额「资金变动」 |

窗口错在哪一边代价不对称，所以宁可开窄：**开太宽**会把市值波动当成转账记进去，盈亏就永久错了，还得人去翻账才发现；**开太窄**只是漏记，手工补一条就行。判定在 `in_capital_sample_window()`，边界有测试钉着。

**盘中转账落在窗口外，不会被自动记录**——这正是 [#2](https://github.com/litaolemo/bigqmt_dashboard/issues/2) 的情况：盘中转出 2 万，面板当日盈亏显示 -2 万左右，而 QMT 里是盈利的。手工补一条 `-20000` 的调整就对上了。

另外「重算历史收益率」按钮在同一个弹窗里：补录了历史出入金之后，用它重算受影响那几天的每日收益率。
...
**通知渠道配置页、国债逆回购开关的前端界面。** 后端和 API 都已完成并测试，但通知渠道要改 `config/notify.json` 或环境变量，逆回购开关要调 `POST /api/account/reverse-repo`，面板上还点不到。（条件单的界面已经做好了，见[条件单](#条件单)。）
